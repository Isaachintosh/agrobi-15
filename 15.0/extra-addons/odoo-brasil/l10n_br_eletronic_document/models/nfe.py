...
class EletronicDocument(models.Model):
    ...
    def _hook_validation(self):
        ...
        if self.model == 'nfe':
            ...
            if self.company_id.l10n_br_account_trustcodeant_id and not \
                self.company_id.l10n_br_account_trustcodeant_id.l10n_br_cnpj_cpf:
        ...
        if item.tipo_produto == 'service':
            ...
            imposto.update({
                'ICMS': {
                    ...
                    'vBCFCPST': "%.02f" % item.icms_st_base_calculo if item.icms_st_base_calculo else '',
                    'pFCPST': "%.02f" % item.fcp_st_aliquota if item.fcp_st_aliquota else '',
                    'vFCPST': "%.02f" % item.fcp_st_valor if item.fcp_st_valor else '',
 ...
 SEM VALOR FISCAL'
        ...
        autorizados = []
        if self.company_id.l10n_br_account_trustcodeant_id:
            autorizados.append({
                'CNPJ': re.sub(
                    '[^0-9]', '', self.company_id.l10n_br_account_trustcodeant_id.l10n_br_cnpj_cpf)
            })
