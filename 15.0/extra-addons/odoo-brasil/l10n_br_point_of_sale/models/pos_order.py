...
class PosOrder(models.Model):
    ...
    sending_nfe = fields.Boolean(
        string="Enviando NFe?", compute="_compute_nfe_number", default=False
    )
    nfe_exception = fields.Boolean(
        string="Problemas na NFe?", compute="_compute_nfe_number", default=False
    )
    nfe_status = fields.Char(
        string="Mensagem NFe", compute="_compute_nfe_number", default=""
    )
    nfe_number = fields.Integer(
        string="Número NFe", compute="_compute_nfe_number", default=0
    )
    nfe_exception_number = fields.Integer(
        string="Número NFe Erro", compute="_compute_nfe_number", default=0
    )
    ...
    def _process_order(self, pos_order, draft, existing_order):
        ...
        if self.env.company.pos_nfce_sync_emission:
            eletronic = self.create_eletronic_invoice(res)
            eletronic.validate_invoice()
            eletronic.action_post_validate()
        return res.id

    def create_eletronic_invoice(self, res):
        self.get_numero_controle(res)
        self.check_fiscal_position_id(res)
        self.get_numero_next_by_id(res)
        self.update_tax_ids(res)
        foo = self._prepare_edoc_vals(res)
        eletronic = self.env["eletronic.document"].create(foo)
        return eletronic

    def get_numero_controle(self, res):
        num_controle = int(
            "".join([str(SystemRandom().randrange(9)) for i in range(8)])
        )
        res.numero_controle = str(num_controle)

    def get_numero_next_by_id(self, res):
        res.numero = res.company_id.l10n_br_nfe_sequence.next_by_id()

    def check_fiscal_position_id(self, res):
        if not res.fiscal_position_id:
            config_id = res.session_id.config_id
            company_id = res.company_id or self.env.company or self.env.user.company_id
            fiscal_position_id = config_id.default_fiscal_position_id or company_id.default_nfce_fiscal_position_id
            res.fiscal_position_id = (
                fiscal_position_id.id
            )

    def update_tax_ids(self, res):
