...
class EletronicDocument(models.Model):
    ...
    def generate_eletronic_document(self, xml_nfe, create_partner):
        ...
        try:
            ...
            raise ValueError(f"Não foi possível codificar o código fonte da nota, favor subir novamente o arquivo.")
        
        invoice_dict.update({
            'nfe_processada': nfe_processada,
            'nfe_processada_name': "NFe%08d.xml" % nfe.NFe.infNFe.ide.nNF
        })
        invoice_dict.update(self.get_protNFe(nfe, company_id))
        invoice_dict.update(self.get_main(nfe))
    ...
    def generate_account_move(self):
        ...
        purchase_order_name = vals.get('name') or f"PO/{vals.get('ref')}"

        purchase_order_id = self._get_purchase_order_vals(purchase_order_name)

        items = []
        for item in self.document_line_ids:
            invoice_item = self.prepare_account_invoice_line_vals(item)
            if item.desconto:
                invoice_item['discount_value'] = item.desconto
        ...
        if self.valor_frete:
            product = self.env.ref("l10n_br_account_trustcode.product_product_delivery")
            items.append((0, 0, self.prepare_extra_line_items(product, self.valor_frete)))

        if self.valor_despesas:
            product = self.env.ref("l10n_br_account_trustcode.product_product_expense")
            items.append((0, 0, self.prepare_extra_line_items(product, self.valor_despesas)))

        if self.valor_seguro:
            product = self.env.ref("l10n_br_account_trustcode.product_product_insurance")
            items.append((0, 0, self.prepare_extra_line_items(product, self.valor_seguro)))

        vals['invoice_line_ids'] = items
        # Injeta tipo de referência da fatura como referência livre
        vals['reference_type'] = 'none'
        ...
        if not purchase_order_id:
            purchase_order_vals = {
                ...
                'date_order': vals.get('date'),
                'date_planned': vals.get('invoice_date'),
                'partner_id': vals.get('partner_id')
            }
            purchase_order_id = self.env["purchase.order"].create(purchase_order_vals)
        
        if not account_invoice.purchase_id:
            account_invoice.purchase_id = purchase_order_id
        if not purchase_order_id.invoice_ids:
            purchase_order_id.invoice_ids = account_invoice
        
        account_invoice.message_post(
            body=f"""
                <ul>
                    <li>Fatura criada através da do xml da NF-e {self.numero}</li>
                    <li>Ordem de Compra {purchase_order_id.name} criada através da do xml da NF-e {self.numero}</li>
                </ul>
            """)
